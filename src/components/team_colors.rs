...
mod tests {
    ...
    const CURRENT_TEAMS: [&str; 30] = [
        "ATH", "SEA", "LAA", "HOU", "TEX", "CWS", "DET", "KC", "MIN", "CLE", "NYY", "BOS", "TOR",
        "TB", "BAL", "LAD", "SF", "AZ", "SD", "COL", "CHC", "STL", "MIL", "CIN", "PIT", "NYM",
        "ATL", "PHI", "MIA", "WSH",
    ];
    ...
    fn all_current_teams_have_colors() {
        for abbr in CURRENT_TEAMS {
            assert!(
                get(abbr, false).is_some(),
                "missing readable color for {abbr}"
            );
            assert!(
                get(abbr, true).is_some(),
                "missing official color for {abbr}"
            );
        }
    }
